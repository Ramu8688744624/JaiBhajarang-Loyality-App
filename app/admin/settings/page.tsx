              ...
              <SectionCard title="Loyalty Rules" icon="💰">
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { label: "Joining Bonus", value: `${form.currency_symbol}${form.joining_bonus}`, color: "#D4A843" },
                      { label: "Redemption %", value: `${form.default_redemption_pct}%`, color: "#2563EB" },
                      { label: "Cashback %", value: `${form.default_cashback_pct}%`, color: "#22C55E" },
                    ].map((s) => (
                      <div key={s.label} className="bg-[#0A0F1E] border border-[#1E2D4A] rounded-xl p-3 text-center">
                  ...
                  <Field label="Joining Bonus Amount"
                    hint="Credited to wallet automatically on first registration">
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#D4A843] font-bold text-sm">
                        {form.currency_symbol}
                      </span>
                      <input type="number" min="0" step="50" className={`${inputCls} pl-8`}
                        value={form.joining_bonus}
                        onChange={(e) => setForm({ ...form, joining_bonus: e.target.value })} />
                    </div>
                  </Field>

                  <Field label="Default Redemption %"
                    hint="% of wallet balance suggested as discount at billing. Admin can override per transaction.">
                    <div className="flex items-center gap-3">
                      <input type="range" min="0" max="100" step="1"
                        value={form.default_redemption_pct}
                    ...
                    <p className="text-xs text-slate-600 mt-1">
                      Example: wallet {form.currency_symbol}1000 → suggests {form.currency_symbol}{(1000 * parseFloat(form.default_redemption_pct || "0") / 100).toFixed(2)} discount
                    </p>
                  </Field>

                  <Field label="Default Cashback %"
                    hint="% of net bill amount credited back to wallet after each purchase">
                    <div className="flex items-center gap-3">
                      <input type="range" min="0" max="30" step="0.5"
                        value={form.default_cashback_pct}
                    ...
                    <p className="text-xs text-slate-600 mt-1">
                      Example: pay {form.currency_symbol}2000 → earn {form.currency_symbol}{(2000 * parseFloat(form.default_cashback_pct || "0") / 100).toFixed(2)} cashback
                    </p>
                  </Field>
