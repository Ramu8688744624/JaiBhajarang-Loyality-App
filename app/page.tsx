"use client";
// app/page.tsx  ─ SMART MARKETING HOMEPAGE
// ══════════════════════════════════════════════════════════════
// Jai Bajrang Mobiles — Karimnagar
// • Detects session: "Customer Login" → "My Dashboard" when signed in
// • Multi-language: English | Telugu | Hindi
// • Responsive: mobile-first with sm/md/lg breakpoints
// • 5+ years · 10,000+ customers · 30-min display change highlight
// ══════════════════════════════════════════════════════════════
...
const T = {
  ...
  te: {
    ...
    services: [
      { icon: "📱", title: "iPhone రిపేర్",         desc: "అన్ని మోడళ్ళకు స్క్రీన్, బ్యాటరీ, కెమెరా రిపేర్. అదే రోజు." },
    ...
    loyalty_desc:  "ప్రతి కొనుగోలుపై క్యాష్‌బ్యాక్ సంపాదించండి. స్నేహితులను రిఫర్ చేసి బోనస్ పొందండి.",
    loyalty_points:[
...
// ══════════════════════════════════════════════════════════════
// SESSION HOOK  (reads from localStorage + Supabase)
// ══════════════════════════════════════════════════════════════
...
function useSession() {
  ...
  useEffect(() => {
    ...
    sb.auth.getSession().then(({ data }) => {
      ...
      if (!uid) return;
      setIsLoggedIn(true);
      sb.from("profiles").select("role").eq("id", uid).single().then(({ data: p }) => {
...
// ══════════════════════════════════════════════════════════════
// INNER PAGE CONTENT  (uses useSearchParams → needs Suspense)
// ══════════════════════════════════════════════════════════════
...
function HomeContent() {
  ...
  return (
    ...
    >
      {/* ── Sticky Navbar ──────────────────────────────────── */}
      <nav
        className="sticky top-0 z-50 border-b border-[#1E2D4A] bg-[#0A0F1E]/96 backdrop-blur-md"
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 flex-shrink-0">
            ...
            <div className="leading-tight">
              <p className="text-sm font-bold leading-none" style={{ color: "#D4A843" }}>
                Jai Bajrang Mobiles
              </p>
              <p className="text-xs text-slate-600 leading-none mt-0.5">{t.city}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            {isLoggedIn ? (
              <Link
                href={dashboardHref}
                className="hidden sm:inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold transition-all active:scale-[0.97]"
      ...
      <section
        ...
        style={{
          background: `
            radial-gradient(ellipse 85% 65% at 15% -5%, rgba(27,58,107,0.65) 0%, transparent 65%),
      ...
      >
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            {lang === "en" ? "Jai Bajrang Mobiles" : lang === "te" ? "జై బజరంగ్ మొబైల్స్" : "जय बजरंग मोबाइल्स"}
          </h1>

          <p className="text-lg sm:text-xl md:text-2xl font-semibold text-slate-300 mb-3">
            {t.tagline}
          </p>
      ...
      {/* ── Stats bar ─────────────────────────────────────── */}
      <section
        className="py-10 px-4 border-y border-[#1E2D4A]"
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
          {t.stats.map((s) => (
            <div key={s.label} className="text-center">
              <p className="text-3xl sm:text-4xl font-black" style={{ color: "#D4A843" }}>
                {s.value}
              </p>
      ...
      <section className="py-10 sm:py-14 px-4">
        <div className="max-w-4xl mx-auto">
          <div
            ...
            style={{
              background: "linear-gradient(135deg,rgba(212,168,67,0.14),rgba(245,208,120,0.06))",
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
            {t.services.map((svc) => (
              <div
                key={svc.title}
                className="bg-[#0F1729] border border-[#1E2D4A] rounded-2xl p-5 sm:p-6 hover:border-[#D4A843]/30 transition-all"
              >
      ...
      <section className="py-12 sm:py-16 px-4" style={{ background: "rgba(27,58,107,0.1)" }}>
        <div className="max-w-5xl mx-auto">
          <div className="grid md:grid-cols-2 gap-10 items-center">
            ...
            {/* Loyalty card visual */}
            <div
              className="bg-[#0F1729] border border-[#1E2D4A] rounded-3xl p-8 text-center"
            >
